...
| ⚡ **盘中实时层** | 常驻引擎订阅 Level-1 快照，动态重排候选、模拟盘自动买卖、异动/买卖点推送 |
...
│  engine 主循环 → feed 订阅 L1 快照 → rerank 动态重排 →      │
│  RankBoard 候选榜 + PaperTrader 模拟盘 → notifier 推送      │
│  (Bark / Server酱 / PushDeer) + ledger 独立账本            │
└──────────────────────────────────────────────────────────┘
```
