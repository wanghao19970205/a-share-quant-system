...
| **V8** | `ask1 / bid1` | 不看模型：20日成交额分位带 `(0.10,0.20]` + 等权 | 只有跌出 `(0.05,0.50]` 才卖，规则同 V7 | `_v8` |
...
订阅是 V7 唯一影响其他账户的地方：带内约占全池 10%（实测 2026-09-03 全池 3194 只、有效截面 3191 只、带内 319 只），不可能全部订阅。`watchlist._read_band()` 只取带内分位最低的 `REALTIME_PAPER_V7_SUBSCRIBE_N` 只（与研究的选股规则一致，实测 40 只覆盖分位 0.3002-0.3124），并使用 `max_subscribe` 之外的独立配额，因此 V1-V6 的候选名额一只不减，代价是行情订阅总数增加约 40 只——2 MB 带宽上限下需要在首日核对 `recv` 与报价新鲜度。持仓照旧由 `config.paper_state_files()` 纳入保护性订阅。

全池分位要读 3000 多个 parquet 尾部，实测 29 秒。`vol_band.rank_pct(cache_dir=...)` 在 `logs/realtime/vol_rank_YYYYMMDD_60.json`（V8 是 `dv_rank_YYYYMMDD_20.json`）落一份当日快照并只保留最近 5 天：进程内缓存救不了盘中 `execv` 重启，而重启可能正好落在 14:50-14:55 买窗里，把所有账户的建仓机会一起拖过去。

口径差不是"滞后一天"：价格文件里已经有当日的 bar，实盘 14:50 读到的是**当日尚未走完的 bar**（当日成交量／20 日均量中位 0.809），研究口径用的是当日完整收盘——既不前视，也不是 T-1。实测带内成员 319 vs 319、交集 284、Jaccard 0.8023，全池分位 Spearman 0.998652，但最大绝对差 0.1377——分位在带下沿很密，"带内最低 40 只"两种口径只重合 16/40。这不改变预期收益：同一条带下 `target_n` 从 20 扫到 319，年化超额 15.89% / 17.52% / 15.33% / 16.07% / 14.53%，留出期 t 全在 3.6 以上，说明超额来自"进了这条带"而不是带内的细微排序。选 20 只的代价是风险调整而非边缘：IR 1.86（n=50 为 2.86），最大回撤 -31.9%。

首日实盘（2026-09-03）：20 只全部建仓，投入 88033 元、闲置现金 12%，入场分位 0.3002-0.3065，单只权重 3.8%-5.6%（高价股只能买 1 手）。订阅从 63 只升到 103 只，`effects_dropped=0`。

调仓间隔只能是逐日：`h=1` 时缓冲带把换手压到 0.034/日，gross +19.94%（t 6.58）、net +15.96%（t 5.35）；改成 `h=10` 后换手反升到 0.232/期，gross 掉到 +9.08%（t 2.56）、net +6.59%（t 1.88）。拉长间隔并不省成本——分位带每天都在动，隔 10 天再看已经漂出去一批，反而要一次性换掉更多仓位，同时错过带内漂移带来的收益。

置换检验（把 `vol_60` 在每个截面内随机打乱 60 次）：**必须用 gross 口径**。带成本的版本零分布均值 -46.19%、`p=0`，但那只是因为随机信号的换手是 0.6624/日（实测 0.0336），`0.004 × 0.66` 按 252 期复利就是 -40% 量级——它检验的是"随机信号换手高"，与选股能力无关。gross 口径下：固定 `(0.30,0.40]` 零分布均值 +5.46%、标准差 3.12%、95 分位 +10.78%，观测 +19.94%（z 4.6）；允许在 7 组带里挑最好（校正多重比较）零分布均值 +9.47%、标准差 2.17%、95 分位 +13.23%，观测 +22.72%（z 6.1）。60 次里都没有一次超过观测值，经验 p < 1/60。

零分布均值不是 0，这一点必须记在口径里：随机信号也能对全池基准跑出 +5.2% 年化。已拆开——池口径（策略要求 `vol_60` 非空）只占 +0.79%，其余约 +4.4% 来自**持仓惯性吃到不可买样本**：调仓日 1.99% 的样本 `buyable_close=False`（多为涨停封板），这批样本的次日收益均值 +1.776%，而可买样本是 -0.010%；`simulate()` 里已持仓不要求可买，于是组合留住了这些票，基准却把它们剔掉了。所以 V7 里**可归因于 `vol_60` 的 gross 超额约 +14.5%**（19.94 - 5.46），扣掉 0.034 × 0.004 × 252 的成本约 **+10.5% 净**，而不是 README 上方的 +15.89%。差值那 5.5pp 在实盘里部分是真的（持有中的涨停股确实拿得到），因此 10.5% 当下限、15.9% 当上限，V7/V8 的前向记录会给出实际落点。

## 8.6 V8 赛马账户（等权低成交额带）

V8 与 V7 机制逐字相同，只把指标换成 20 日均成交额（`volume × close`，`min_periods=15`）：进 `(0.10,0.20]`、跌出 `(0.05,0.50]` 才卖、等权 20 只。研究口径（2089 个交易日、成本 0.004、n=20）年化超额 +17.97%，IS +17.85%（t 4.32）、OOS +17.23%（t 2.87）。

开成独立账户而不是并入 V7 的唯一理由是两条腿几乎不重合：每个调仓日候选前 20 的 Jaccard 均值 0.0032（中位 0），日度超额相关系数 0.174，五五等权合并后 IR 从 1.86/1.78 升到 2.37。前向跑两个独立账户，才能用真实成交检验这个相关性。

```text
REALTIME_PAPER_V8=true
REALTIME_PAPER_V8_POSITIONS=20
...
REALTIME_PAPER_V8_SUBSCRIBE_N=40
```

两个必须盯住的风险：

- **"低成交额"不等于"买不进去"，但价差要实测。** 带内 20 日均成交额中位 4237 万元、最小 3455 万元，单笔 5000 元只占 0.012%，99.1% 买得起 1 手——主板活跃池本身筛掉了僵尸股。真正的不确定性是买卖价差，`buy_last/bid1/ask1` 与 `sell_last/bid1/ask1` 就是为此记录的。
- **参数稳健性弱于 V7。** 进带扫描里最强的是最边缘那一档 `(0.10,0.20]`，中间档 `(0.25,0.35]` 留出期 t 只有 1.94，不像 vol60 那样呈连片平台。所以 V8 的定位是前向验证，不是已确认的结论。
...
| `paper_*_v8.json(l)` | V8 等权低成交额带账户的同名独立副本 |
...
当前虚拟数据流回归为 **149 PASS / 0 FAIL**，覆盖模型池封闭、重排有界、要求有效实时价且排除封涨停票的行业/概念 Top5 Push、绕过行情 SDK stop 的安全热重载、A 股 T+1、五级卖出、保护性订阅、融合预期成本安全边际门、二阶段买入及跨重启恢复、决策 trace 和反事实幂等补齐；V1-V5 的 ATR 风险预算、融合收益有限加权和整手约束；V4 行业集中度；V5 strong/neutral/lagging 动态仓位、缺行情降级、独立账户和保护性订阅；四版基线同场买卖完整生命周期；NaN/Inf、全模型异常、无效盘口、损坏状态文件故障注入；以及 realtime-only 滚动约束、未兑现日期排除、进化权重目标诊断、40 日 OOS、4 worker 确定性、自动晋级、损坏回退、幂等执行和 10 日前向回滚；V7 的带内升序建仓、等权整手、跌出出带才卖（含"分位取不到即继续持有"）、T+1、独立账户文件和独立订阅配额；V8 的指标切换、独立分位表与磁盘快照、独立出带与中文出场原因、以及 V7/V8/模型候选三份订阅配额互不挤占。
