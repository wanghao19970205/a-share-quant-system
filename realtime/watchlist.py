...
def _read_band(cfg: RealtimeConfig, prefix: str, metric: str,
    ...
    升序取 need 只）。实盘 14:50 读到的是当日还没走完的 bar（不是 T-1 收盘，价格文件
    里已有当日数据；当日量/20 日均量中位 0.809），研究算的是当日完整收盘，实测"带内
    最低 40 只"两种口径只重合 16/40；但同一条带下 target_n 从 20 扫到 319 的年化超额
    在 14.5%-17.5% 之间、留出期 t 全在 3.6 以上，超额来自"进了这条带"而不是带内排序，
    所以这个截断和这点口径差都不改变预期。数量必须有界：带内约占全池 10%（数百只），
