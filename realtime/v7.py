...
持仓数定 20 而不是研究里风险调整更好的 40-50。实测（2026-09-03 带内 319 只，中位价
8.74 元、90 分位 27.31 元）：

- n=20 每只 5000 元，97.5% 买得起 1 手，整手取整后实际投入/目标中位 0.93；
- n=30 每只 3333 元 → 93.4% / 0.89；n=40 → 88.1% / 0.88；n=50 → 83.1% / 0.83。

买不起的都是高价股，抬高持仓数等于系统性地把组合从高价股一侧削掉，而研究是带内等权；
同时闲置现金从 7% 涨到 11%-17%，直接按比例摊薄超额。研究侧的收益差在噪声量级
（n=20/30/40/50 年化超额 15.89/17.72/18.15/17.52%，留出期 15.44-16.73%），换来的
IR 提升（1.86 → 2.39 → 2.71 → 2.86）是分散化的真实收益，但代价是口径偏离和订阅
配额翻倍。权衡后取 20：留出期超额恰好也是这一档最高（+16.70%，t 3.60）。
"""
